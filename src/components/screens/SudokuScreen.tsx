...
function pickPuzzle(): SudokuPuzzle {
...
export function SudokuScreen() {
  ...
  const [puzzle, setPuzzle] = useState<SudokuPuzzle>(pickPuzzle)
  const [board, setBoard] = useState<Cell[]>(() => parseBoard(puzzle))
  ...
  }, [selected, notesMode, puzzle.solution, solved])
  ...
  }, [selected, puzzle.solution, solved, hintsLeft])
  ...
  const reset = (samePuzzle = false) => {
    const next = samePuzzle ? puzzle : pickPuzzle()
    setPuzzle(next)
    ...
    <main className="app-shell sudoku-shell">
      ...
      <section className="sudoku-layout">
        <article className="sudoku-hero">
          <div>
            <p className="eyebrow">Number logic · {puzzle.difficulty}</p>
          ...
          <div className="sudoku-actions">
            ...
            <button type="button" onClick={() => reset(false)}>New puzzle</button>
          </div>
        </article>
