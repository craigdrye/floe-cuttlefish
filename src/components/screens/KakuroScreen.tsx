...
export function KakuroScreen() {
  ...
  const daily = useDailyPuzzle('kakuro')
  // Kakuro only has one hand-validated puzzle, so daily and bonus share the same board —
  // the daily/bonus loop just tracks attempts and gating, not puzzle variation.
  const [board, setBoard] = useState<CellState[][]>(freshBoard)
  const [puzzleMode, setPuzzleMode] = useState<PuzzleMode>('daily')
  const [selected, setSelected] = useState<[number, number] | null>(null)
  const [mistakes, setMistakes] = useState(0)

  // If the player solved the daily on a previous visit, drop them into the locked/bonus state
  // rather than letting them re-solve and re-mark the daily.
  const cardOpen = daily.dailyComplete && (daily.phase === 'bonus' || daily.phase === 'locked')
    ...
    <main className="app-shell kakuro-shell">
      ...
      <section className="kakuro-layout">
        ...
        <section className="kakuro-stage">
            ...
            <div className="kakuro-result">
              ...
              {daily.phase === 'bonus' && (
                <button type="button" onClick={startBonusPuzzle}>
                  Replay puzzle ({daily.bonusRemaining} bonus left)
                </button>
              )}
              {daily.phase === 'locked' && (
          ...
          {!solved && cardOpen && daily.phase === 'locked' && (
            <div className="kakuro-result">
