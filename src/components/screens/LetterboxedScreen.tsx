...
import { ArrowLeft, RotateCcw, Trophy, X, HelpCircle } from 'lucide-react'
...
function pickPuzzle(): LetterboxedPuzzle {
...
export function LetterboxedScreen() {
  ...
  const [puzzle, setPuzzle] = useState<LetterboxedPuzzle>(pickPuzzle)
        ...
        <section className="letterboxed-stage">
          ...
          {status === 'won' && (
            <div className="letterboxed-result">
              <Trophy size={18} />
              <strong>Solved in {solvedWords.length}.</strong> Par was {puzzle.par}.
