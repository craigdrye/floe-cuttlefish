...
import { ArrowLeft, Delete, HelpCircle, RotateCcw } from 'lucide-react'
...
function buildEquation(pattern: number): string | null {
  if (pattern === 0) {
    const a = 10 + Math.floor(Math.random() * 90)
    const b = 10 + Math.floor(Math.random() * 90)
    const op = OPS[Math.floor(Math.random() * 3)] // skip /
...
function generateSolution(): string {
  for (let attempts = 0; attempts < 2000; attempts += 1) {
    const eq = buildEquation(Math.floor(Math.random() * 4))
...
export function NerdleScreen() {
  ...
  }, [current, guesses, solution, status, showToast])
        ...
        <article className="nerdle-hero">
          ...
          <div className="nerdle-actions">
            ...
            <button type="button" onClick={reset}>
              <RotateCcw size={16} /> New equation
            </button>
          </div>
        </article>
        ...
        <section className="nerdle-stage">
          ...
          {status === 'won' && (
            <div className="nerdle-result win">
              <strong>Solved!</strong> {guesses.length} {guesses.length === 1 ? 'try' : 'tries'}.
              <button type="button" onClick={reset}>New equation</button>
            </div>
          )}
          {status === 'lost' && (
