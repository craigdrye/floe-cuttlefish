...
import { ArrowLeft, Delete, Check, HelpCircle } from 'lucide-react'
...
function pickRandomSolutions(n: number): string[] {
...
function pickDailySolutions(gameId: DailyGameId, dateKey: string, n: number): string[] {
  const rng = dailyRng(gameId, dateKey)
  const picked = new Set<string>()
  while (picked.size < n) {
    const idx = Math.floor(rng() * wordleWords.length)
    picked.add(wordleWords[idx].toUpperCase())
  }
  return [...picked]
}
...
export function MultiWordleScreen({ boardCount, title, subtitle, gameId }: MultiWordleScreenProps) {
  const { setScreen } = useStore()
  const daily = useDailyPuzzle(gameId)
  ...
  useEffect(() => {
    ...
    if (daily.phase === 'daily') {
      setSolutions(pickDailySolutions(gameId, daily.date, boardCount))
      setPuzzleMode('daily')
      setGuesses([])
      setCurrent('')
    }
  }, [daily.phase, daily.date, solutions, gameId, boardCount])

  const activeSolutions = useMemo(() => solutions ?? [], [solutions])
  ...
  }, [guesses, activeSolutions])

  const allSolved = activeSolutions.length > 0 && solvedAt.every((s) => s !== null)
  const outOfGuesses = guesses.length >= maxGuesses
  const status: GameStatus = solutions === null
    ? 'playing'
    : allSolved ? 'won' : outOfGuesses ? 'lost' : 'playing'
  ...
  const evaluationsPerBoard: LetterState[][][] = useMemo(() => {
    return activeSolutions.map((sol, boardIdx) => {
  ...
  }, [guesses, activeSolutions, solvedAt])
  ...
  const startBonusPuzzle = useCallback(() => {
    ...
    daily.consumeBonus()
    setSolutions(pickRandomSolutions(boardCount))
    setPuzzleMode('bonus')
    setGuesses([])
    setCurrent('')
    setToast(null)
      ...
      <section className="wordle-layout">
        ...
        <section className="wordle-stage">
          ...
          <div className={`multi-wordle-grid ${sizeClass}`}>
            {activeSolutions.map((sol, boardIdx) => {
          ...
          {solutions === null && daily.phase === 'bonus' && (
            <div className="wordle-result wordle-result-win">
              <strong>Daily already solved.</strong> Want a bonus round? {daily.bonusRemaining} left today.
              <button type="button" onClick={startBonusPuzzle}>Play bonus round</button>
            </div>
          )}
          {solutions === null && daily.phase === 'locked' && (
          ...
          {status === 'won' && solutions !== null && (
            <div className="wordle-result wordle-result-win">
              <strong>{puzzleMode === 'daily' ? `Daily ${title.toLowerCase()} cleared!` : `Cleared all ${boardCount}.`}</strong> Used {guesses.length} of {maxGuesses} guesses.
              {daily.phase === 'bonus' && (
                <button type="button" onClick={startBonusPuzzle}>
                  Play bonus round ({daily.bonusRemaining} left)
                </button>
              )}
              {daily.phase === 'locked' && (
...
export function QuordleScreen() {
  return <MultiWordleScreen boardCount={4} title="Quordle" subtitle="Four words. Nine guesses." gameId="quordle" />
}

export function OctordleScreen() {
  return <MultiWordleScreen boardCount={8} title="Octordle" subtitle="Eight words. Thirteen guesses." gameId="octordle" />
}
