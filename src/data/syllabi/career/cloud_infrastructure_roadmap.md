...
## Research Notes
The course is deliberately concept-first and vendor-neutral; the following authoritative sources were used to confirm definitions, weights, and current practice. All were verified current as of 2026.
- Google SRE — Alerting on SLOs (https://sre.google/workbook/alerting-on-slos/): canonical SLI/SLO/error-budget and multi-window burn-rate alerting; confirms burn rate as the escalation signal in Chapter 5.
- Kubernetes docs — Configure Liveness, Readiness and Startup Probes (https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/): authoritative probe semantics (liveness restarts; readiness removes from Service endpoints without restart; startup gates the other two) underpinning Chapter 4.
- Veeam — The 3-2-1 Backup Rule (https://www.veeam.com/blog/321-backup-rule.html) and Opti9 — The 3-2-1-1-0 Backup Strategy (https://opti9tech.com/blog/the-3-2-1-1-0-backup-strategy-explained/): confirm the 3-2-1-1-0 evolution (+1 immutable/air-gapped copy, 0 verified-restore errors) used in Chapter 7.
- FinOps Foundation framework via Flexera — FinOps principles (https://www.flexera.com/blog/finops/finops-principles/): rate vs. usage optimization (commitment discounts, rightsizing, scheduling, hidden egress) anchoring Chapter 6.
- PagerDuty — Best practices to reduce MTTR (https://www.pagerduty.com/resources/incident-management-response/learn/best-practices-to-reduce-mttr/): confirms MTTD/MTTA/MTTR, severity tiers, incident-commander role, and blameless postmortems in Chapter 8.
