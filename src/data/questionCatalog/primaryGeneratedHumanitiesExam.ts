...
const grade4ReadingBlueprints: Blueprint[] = [
  ...
  },
  {
    ...
    wrong: [
      ...
      miss(`The article is mainly a list of jokes.`, 'Nothing in the prompt suggests jokes.', 'Use the stated contents.'),
    ],
  ...
  const banks: Record<number, Blueprint[]> = {
    ...
    4: [
      ...
      },
      {
        ...
        wrong: [
          miss('How many letters are in sculpture?', 'That is a spelling/counting question.', 'Philosophy asks about meaning and criteria.'),
          miss('Can the object be thrown faster?', 'That is unrelated to art value.', 'Focus on the concept of art.'),
          miss('Is every sculpture made on Tuesday?', 'The day is irrelevant.', 'Ask a question about definition or value.'),
        ],
    ...
    5: [
      ...
      },
      {
        ...
        wrong: [
          miss('Whether it is shiny.', 'Appearance is not the ethical issue.', 'Rights questions concern moral status.'),
          miss('Whether it is expensive only.', 'Cost alone does not decide rights.', 'Ask about capacities and harm.'),
          miss('Whether it can win a race.', 'Speed is not enough for moral consideration.', 'Consider consciousness or interests.'),
        ],
      ...
      },
      {
        ...
        wrong: [
          miss('Are far-away people imaginary?', 'People in other countries are real.', 'Global ethics expands the circle of concern.'),
          miss('Does helping nearby prove far-away help is wrong?', 'The two duties may need comparison.', 'Ask how responsibilities are weighed.'),
          miss('Should no one ever help?', 'That avoids the moral question.', 'The scenario asks about obligation.'),
        ],
      ...
      },
      {
        ...
        wrong: [
          miss('Where we grow up never affects us.', 'The scenario shows support affecting confidence.', 'Consider nurture.'),
          miss('People have no memories.', 'The prompt is about environment, not memory loss.', 'Identify the factor being tested.'),
          miss('Confidence is a type of weather.', 'Confidence is a personal trait.', 'Stay with the identity concept.'),
        ],
    ...
    6: [
      ...
      },
      {
        ...
        wrong: [
          ...
          miss('Pure selfishness', 'Utilitarianism counts overall welfare, not only one person.', 'Consider everyone affected.'),
          miss('Spelling practice', 'This is an ethics question.', 'Use the framework names.'),
        ],
      ...
      },
      {
        ...
        wrong: [
          miss('A random superstition', 'The statement gives a political reason for rules.', 'Social contract explains why authority may be accepted.'),
          miss('A metaphor only about weather', 'The idea is about society, not weather.', 'Track the concept.'),
      ...
      },
      {
        ...
        wrong: [
          miss('The social contract', 'That is about government and rules.', 'This question is about machine intelligence.'),
          miss('The trolley problem', 'That is an ethics dilemma about harm.', 'Match the AI thought experiment.'),
          miss('A weather forecast', 'No weather is being predicted.', 'Use the philosophy of mind concept.'),
        ],
      ...
      },
      {
        ...
        wrong: [
          miss('Can machines only make toast?', 'The scenario is a philosophical thought experiment.', 'Ask about value and meaning.'),
          miss('Should spelling tests be longer?', 'That is unrelated to the good life.', 'Use the details about pleasure and reality.'),
          miss('Are relationships always fake?', 'The prompt contrasts simulated pleasure with real relationships.', 'Do not overgeneralize.'),
        ],
...
const naplanYear5Blueprints: Blueprint[] = [
  ...
  },
  {
    ...
    correct: 'However,',
    wrong: [
      miss('Meow,', 'This is not a logical transition.', 'Transitions show relationships between ideas.'),
      miss('Under the chair,', 'This is a place phrase, not a contrast transition.', 'Choose a word that signals contrast.'),
      miss('Because and because,', 'This is not a clear standard transition.', 'Use concise linking words.'),
    ],
...
const codingBlueprints: Blueprint[] = [
  ...
  },
  {
    ...
    wrong: [
      miss('Only an insect on a desk', 'In coding, bug has a special meaning.', 'Use the computing definition.'),
      miss('A finished program with no problems', 'A bug means something is wrong.', 'Debugging fixes mistakes.'),
      miss('A type of sandwich', 'That is unrelated to coding.', 'Choose the coding term.'),
    ],
  ...
  },
  {
    ...
    wrong: [
      miss('Tie shoes before wearing them.', 'You cannot tie shoes usefully before putting them on.', 'Debug by finding the wrong order.'),
      miss('Delete socks forever.', 'That avoids rather than fixes the sequence.', 'Fix the instruction, not the whole task.'),
      miss('Add five unrelated jumps.', 'Extra unrelated steps do not solve the bug.', 'Make the smallest useful correction.'),
    ],
  ...
  },
  {
    ...
    wrong: [
      miss('Take sunglasses because it is always sunny.', 'The condition says raining.', 'Apply the if/then rule.'),
      miss('Do nothing because rules are ignored.', 'Computers follow instructions exactly.', 'Use the condition.'),
      miss('Paint the umbrella', 'The rule says take it, not paint it.', 'Choose the specified action.'),
    ],
  ...
  },
  {
    ...
    wrong: [
      miss('The switch is OFF.', 'ON and OFF are opposite states.', 'Binary choices often have two clear states.'),
      miss('The switch is both ON and OFF in this simple example.', 'The prompt gives one state.', 'Use the stated condition.'),
      miss('The switch is a sandwich.', 'That is unrelated.', 'Choose a logical statement.'),
    ],
