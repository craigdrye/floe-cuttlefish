...
export function defaultQuestionScaffold(topic: Topic, title: string, prompt: string) {
  ...
  const byTopic: Record<Topic, { briefing: string; setup: string[]; fieldNote: string; mentorHint: string }> = {
    ...
    Science: {
      briefing: 'This is a science question. Identify the system, the variable that changes, and the mechanism before choosing an answer.',
      setup: [
        `You are working on "${title}".`,
        'Separate observation from explanation before reading the choices.',
        'Prefer answers that respect mechanism, scale, and the stated conditions.',
      ],
      fieldNote: 'Science questions reward naming the principle (e.g., conservation, equilibrium, gradient) before doing the calculation.',
      mentorHint: 'Ask what is being conserved, what is changing, and what unit the answer should be in.',
    },
    High: {
      briefing: 'This is a high-school core question. Use the named definition, formula, or principle directly rather than nearby vocabulary.',
      setup: [
        `You are working on "${title}".`,
        'Pull out the term the question is testing before scanning the answers.',
        'The right answer should match the textbook definition, not a near-synonym.',
      ],
      fieldNote: 'Many wrong answers on high-school items are adjacent terms that sound right but mean something different.',
      mentorHint: 'State the definition first, then match the option that fits it.',
    },
  }
...
export function makeSimpleQuestion(
  ...
  return {
    ...
    // Setup is intentionally omitted: the scaffold's generic boilerplate
    // (e.g. "You are working on \"X\". Identify the core concept...") adds
