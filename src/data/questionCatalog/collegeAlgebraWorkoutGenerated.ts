...
const _baseCollegeAlgebraWorkoutGeneratedQuestions = makeQuestionBank('Mathematics', [
  ...
  q(392081, 'Linear Equations', 'Distribute and solve', 'Solve 2(3x - 4) = 10.', 'x = 3', [
    miss('x = 1', 'That comes from solving 3x - 4 = 10 without distributing correctly.', 'Distribute the 2 first.'),
    miss('x = 14/6', 'That stops before simplifying fully.', 'After distributing, isolate x completely.'),
    miss('x = 5', 'That treats 6x as 2x or skips subtracting -8.', 'Write 6x - 8 = 10 first.'),
    miss('x = -3', 'The sign flips incorrectly during solving.', 'Check by substitution into the original equation.'),
  ]),
  q(392082, 'Quadratics', 'Roots from graph meaning', 'If a parabola crosses the x-axis at x = -1 and x = 4, which factorization matches?', '(x + 1)(x - 4)', [
    miss('(x - 1)(x + 4)', 'Those roots would be 1 and -4.', 'Each x-intercept makes one factor zero.'),
    miss('(x + 1)(x + 4)', 'That gives roots -1 and -4.', 'Use the sign opposite each root.'),
    miss('(x - 1)(x - 4)', 'That gives roots 1 and 4.', 'A root at -1 needs x + 1.'),
    miss('x(x + 1)(x - 4)', 'That introduces an extra root at 0.', 'Use exactly the two given roots.'),
  ]),
  q(392083, 'Functions', 'Domain from denominator', 'What is the domain restriction for f(x) = 1/(x + 6)?', 'x != -6', [
    miss('x != 6', 'That uses the wrong sign when solving x + 6 = 0.', 'Set the denominator equal to zero carefully.'),
    miss('x > -6 only', 'Values less than -6 are allowed as long as the denominator is not zero.', 'Exclude just the zero-denominator point.'),
    miss('All real numbers', 'Rational functions exclude values that zero the denominator.', 'The denominator cannot be zero.'),
    miss('x != 0', '0 is allowed here because 0 + 6 is not zero.', 'Check the actual denominator.'),
  ]),
  q(392084, 'Exponents', 'Zero exponent', 'For nonzero a, a^0 equals:', '1', [
    miss('0', 'A zero exponent does not make the value zero.', 'Use the exponent rule for equal-base division.'),
    miss('a', 'That is a^1, not a^0.', 'Dropping the exponent is not correct here.'),
    miss('Undefined for every a', 'It is defined for nonzero a.', 'The only excluded case here is a = 0.'),
    miss('1/a', 'That corresponds to a^-1.', 'Negative one and zero exponents are different.'),
  ]),
  q(392085, 'Probability', 'Simple probability', 'A bag has 3 red marbles and 7 blue marbles. What is the probability of drawing a red marble on one draw?', '3/10', [
    miss('7/10', 'That is the probability of blue, not red.', 'Use favorable outcomes over total outcomes.'),
    miss('3/7', 'That compares red to blue instead of red to total.', 'Total marbles is 10.'),
    miss('1/3', 'That does not match the counts in the bag.', 'Count all marbles, not just colors.'),
    miss('10/3', 'Probabilities cannot exceed 1.', 'A probability is a fraction of the total.'),
  ]),
  q(392086, 'Linear Models', 'Slope from equation', 'What is the slope of the line y = 5x - 2?', '5', [
    miss('-2', 'That is the y-intercept, not the slope.', 'In y = mx + b, the slope is m.'),
    miss('2', 'That uses the intercept with the wrong sign.', 'Look at the coefficient of x.'),
    miss('-5', 'The coefficient of x is positive here.', 'Check the sign on 5x.'),
    miss('0', 'This line is not horizontal.', 'A nonzero x-coefficient means nonzero slope.'),
  ]),
  q(392087, 'Inequalities', 'Graph description', 'Which graph matches x > -2?', 'An open circle at -2 with shading to the right', [
    miss('A closed circle at -2 with shading to the right', 'Closed circles include the endpoint, but > does not.', 'Strict inequalities use open circles.'),
    miss('An open circle at -2 with shading to the left', 'That would represent x < -2.', 'Greater than points right on the number line.'),
    miss('A closed circle at -2 with shading to the left', 'That matches x <= -2.', 'Check both the direction and whether the endpoint is included.'),
    miss('A single filled point at -2 only', 'That would represent x = -2.', 'Inequalities usually describe intervals, not one point.'),
  ]),
  q(392088, 'Quadratics', 'Standard form identification', 'Which equation is in quadratic standard form?', 'y = 2x^2 - 3x + 7', [
    miss('y = (x - 1)(x + 4)', 'That is factored form, not standard form.', 'Standard form looks like ax^2 + bx + c.'),
    miss('y - 7 = 2x(x - 1)', 'That is not written in expanded standard form.', 'Expand and arrange by powers of x.'),
    miss('x = 2y^2 - 3y + 7', 'That is quadratic in y, not standard form for y as a function of x.', 'Use y in terms of x.'),
    miss('y = 2/x^2 - 3x + 7', 'That is not a quadratic polynomial because of the denominator.', 'Quadratics use nonnegative integer powers of x only.'),
  ]),
  q(392089, 'Functions', 'Evaluate from table', 'A table shows x = 4 maps to f(x) = -1. What is f(4)?', '-1', [
    miss('4', 'That repeats the input value instead of the output.', 'Read the function value from the table.'),
    miss('1', 'The sign of the output is wrong.', 'Check whether the table says 1 or -1.'),
    miss('0', 'That is not the listed output.', 'Use the given row of the table.'),
    miss('Undefined', 'The function value is provided directly.', 'If the table includes x = 4, then f(4) is defined there.'),
  ]),
  q(392090, 'Probability', 'Expected count', 'If the probability of success is 0.2 on each of 50 independent trials, the expected number of successes is:', '10', [
    miss('0.1', 'That divides instead of multiplying.', 'Expected count is n times p.'),
    miss('20', 'That treats 0.2 as 20 successes out of 50 incorrectly.', 'Compute 50 * 0.2.'),
...
export const collegeAlgebraWorkoutGeneratedQuestions = runPolish(_baseCollegeAlgebraWorkoutGeneratedQuestions, [
  {
