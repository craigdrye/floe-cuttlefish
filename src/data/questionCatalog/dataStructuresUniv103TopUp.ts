...
export const dataStructuresUniv103TopUpQuestions: Question[] = makeQuestionBank('University', [
  // ---------------- Chapter 1: Arrays and Dynamic Arrays ----------------
  {
    ...
    mentorHint: 'Amortized analysis asks you to charge rare expensive resizes across the many cheap appends between them. With doubling, look at the total number of elements copied over a long sequence, then divide that total work by the number of appends.',
  ...
  },
  {
    ...
    mentorHint: 'Compare how often resizing happens under fixed-size growth versus multiplicative growth. If capacity increases by only k each time, the copies get larger while the number of cheap appends between copies stays bounded.',
  ...
  // ---------------- Chapter 2: Linked Lists ----------------
  {
    ...
    mentorHint: 'Separate the cost of finding a position from the cost of changing links once the position is known. In a singly linked list, insertion after a known node only needs a small, fixed number of pointer assignments and does not move existing nodes.',
  ...
  },
  {
    ...
    mentorHint: 'Floyd cycle detection trades stored history for relative motion: two references move through the same list at different speeds. Focus on how many variables the algorithm keeps, not how many nodes it may visit before the pointers meet or the list ends.',
  ...
  // ---------------- Chapter 3: Stacks and Queues ----------------
  {
    ...
    mentorHint: 'A two-stack queue is another amortized-analysis problem: an element may be moved during a batch transfer, but it cannot bounce back and forth indefinitely. Track the lifetime of one element across pushes, transfers, and pops to estimate total work over many operations.',
  ...
  },
  {
    ...
    mentorHint: 'A stack is best when the most recently seen unfinished item must be handled first. Look for a problem with nested structure, where closing or completing something should match the latest still-open item.',
  ...
  // ---------------- Chapter 4: Hash Tables ----------------
  {
    ...
    mentorHint: 'Open addressing stores all records inside the table array, so a collision means searching for another slot in the same finite space. As occupancy rises, reason about how long contiguous occupied runs affect future probes, especially with linear probing.',
  ...
  },
  {
    ...
    mentorHint: 'Load factor is a measure of table crowding, not a direct count of collisions. For chaining, interpret it as the average number of entries per bucket under a reasonably uniform hash function.',
  ...
  // ---------------- Chapter 5: Trees and Binary Search Trees ----------------
  {
    ...
    mentorHint: 'Use the BST invariant recursively: everything on one side of a node is smaller, and everything on the other side is larger. Then combine that invariant with the visitation order of an in-order traversal.',
  ...
  },
  {
    ...
    mentorHint: 'BST search follows one root-to-leaf path, so the key variable is tree height. Ask what shape a plain BST can take when insertions are unlucky and no rotations or balancing rules repair the shape.',
  ...
  // ---------------- Chapter 6: Balanced Trees ----------------
  {
    ...
    mentorHint: 'AVL trees enforce a local height rule at every node, and that local rule prevents long skinny branches from forming globally. Think in terms of subtree heights and the balance factor, not exact equality of node counts.',
  ...
  },
  {
    ...
    mentorHint: 'Red-black trees encode balance with color constraints rather than exact height differences. Look for rules that limit consecutive red nodes and keep the number of black nodes consistent along root-to-leaf paths.',
  ...
  // ---------------- Chapter 7: Heaps and Priority Queues ----------------
  {
    ...
    mentorHint: 'Bottom-up heapify does not give every node a full-height sift. Most nodes are near the leaves, so estimate total work by grouping nodes by height and multiplying how many there are by how far they can move.',
  ...
  // ---------------- Chapter 8: Graphs and Representations ----------------
  {
    ...
    mentorHint: 'Choose a graph representation by matching storage to the graph density and query pattern. Sparse graphs have far fewer actual edges than possible edges, so compare storing only neighbors with reserving space for every possible vertex pair.',
  ...
  // ---------------- Chapter 9: Tries ----------------
  {
    ...
    mentorHint: 'A trie routes the search by characters, so the path length is tied to the query string itself. Distinguish dependence on key length from dependence on how many other keys happen to share the data structure.',
