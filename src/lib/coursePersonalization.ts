...
export function stageDetailForNumericAge(age: number): StageDetail | null {
  // Floor is age 8 post-2026-05-18 cut (preschool tier removed; primary Year 1-2 removed).
  if (age <= 10) return `primary-year-${Math.min(6, Math.max(3, age - 5))}` as StageDetail
