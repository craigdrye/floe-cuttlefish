...
SCRATCH_REPO = REPO_ROOT / "scratch" / "openintro_labs_rguroo_repo"
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "openintro_labs_rguroo"
OUTPUT_JSON = OUTPUT_DIR / "openintro_labs_rguroo_tasks.json"
OUTPUT_MD = OUTPUT_DIR / "openintro_labs_rguroo_summary.md"
REPO_URL = "https://github.com/OpenIntroStat/openintro-labs-rguroo.git"
...
def infer_candidate_tracks(lab_slug: str, prompt: str) -> list[str]:
    ...
    if any(key in lowered for key in ["probability", "proportion", "confidence interval", "hypothesis", "sampling"]):
        candidates.extend(["quant", "quantAdvanced"])
    if any(key in lowered for key in ["plot", "visualize", "distribution", "histogram", "boxplot"]):
...
def extract_all_tasks() -> list[ExtractedTask]:
    ...
    for rmd_file in sorted(SCRATCH_REPO.glob("*/*.Rmd")):
        ...
        for question_number, block in extract_numbered_blocks(text):
            ...
            if len(prompt) < 25:
                continue
            source_id = f"openintro-labs-rguroo::{lab_slug}::{question_number}"
            collection_id = f"openintro-labs-rguroo::{lab_slug}"
            tasks.append(
                ExtractedTask(
                    source_id=source_id,
...
def write_outputs(tasks: list[ExtractedTask]) -> None:
    ...
    lines = [
        "# OpenIntro Labs Rguroo Raw Collection",
        "",
        "- Source: OpenIntro Labs (Rguroo)",
        "- License: CC BY-SA 4.0",
...
def main() -> None:
    ...
    write_outputs(tasks)
    print(f"Extracted {len(tasks)} Rguroo lab tasks into {OUTPUT_JSON}")
