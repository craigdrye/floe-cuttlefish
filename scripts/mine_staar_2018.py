...
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "staar_2018"
OUTPUT_JSON = OUTPUT_DIR / "staar_2018_questions.json"
OUTPUT_MD = OUTPUT_DIR / "staar_2018_summary.md"
...
def collect_test_pairs() -> list[tuple[str, str, str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if "writing-samples" in lowered_href or "revising-and-editing" in lowered_href:
            continue
        if "2018-staar-" not in lowered_href and "staar-2018-" not in lowered_href:
            continue
        if "rationale" in lowered_href:
            continue

        title_match = re.match(r"(.+?)\s+(Online\s+Answer\s+Key|Paper\s+Test\s+Form|Paper\s+Answer\s+Key)\s*$", text)
        if not title_match:
            continue
        title = title_match.group(1).strip()
        kind = title_match.group(2).strip()

        if kind == "Paper Test Form":
            tests[title] = href
        elif kind == "Paper Answer Key":
            keys[title] = href
        elif kind == "Online Answer Key" and title not in keys:
            keys[title] = href

        if "released-test.pdf" in lowered_href and "reading" in lowered_text:
            tests[title] = href
...
def build_questions() -> list[ExtractedQuestion]:
    ...
    for title, test_url, key_url in collect_test_pairs():
        ...
        collection_id = f"staar-2018::{re.sub(r'[^a-z0-9]+', '-', title.lower()).strip('-')}"
        ...
        for question_number, prompt_text in sorted(questions.items()):
            items.append(
                ExtractedQuestion(
                    ...
                    topic_tags=[title, subject, grade_band, "staar-2018"],
                )
            )
    return items
...
def write_outputs(items: list[ExtractedQuestion]) -> None:
    ...
    payload = {
        "source": "Texas STAAR 2018 released tests",
...
def main() -> None:
    ...
    write_outputs(items)
    print(f"Extracted {len(items)} STAAR 2018 questions into {OUTPUT_JSON}")
