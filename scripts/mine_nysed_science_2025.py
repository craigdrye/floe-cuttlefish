...
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "nysed_science_2025"
OUTPUT_JSON = OUTPUT_DIR / "nysed_science_2025_questions.json"
OUTPUT_MD = OUTPUT_DIR / "nysed_science_2025_summary.md"
USER_AGENT = "floe-rebuild8-content-miner/1.0"

GRADE_URLS = {
    5: "https://www.nysedregents.org/ei/science/2025/2025-released-items-science-g5.pdf",
    8: "https://www.nysedregents.org/ei/science/2025/2025-released-items-science-g8.pdf",
}
...
def split_questions(text: str) -> dict[int, str]:
    ...
    for index in range(1, len(parts), 2):
        ...
        if body and "Question Type Key Points Performance" not in body:
...
def split_item_map_answers(text: str) -> dict[int, str]:
    ...
    pattern = re.compile(
        r"(?<!\d)(\d{1,2})\s+(Multiple Choice|Constructed Response)\s+([A-D]|n/a)\s+(\d)\s+([A-Z0-9\-\.]+)\s+",
        flags=re.IGNORECASE,
    )
    for match in pattern.finditer(cleaned):
        ...
        expectation = match.group(5)
        answers[number] = f"{qtype}; correct={correct}; points={points}; standard={expectation}"
    return answers


def infer_candidate_tracks(grade: int) -> list[str]:
    if grade == 5:
        return ["scienceYear5", "ck12", "khanacademy"]
    return ["middleSchoolEarthSpace", "ck12", "khanacademy"]
...
def build_questions() -> list[ExtractedQuestion]:
    ...
    for grade, url in GRADE_URLS.items():
        ...
        exam_title = f"NYSED 2025 Grade {grade} Science"
        collection_id = f"nysed-science-2025::grade-{grade}"
        for question_number, prompt_text in sorted(questions.items()):
            items.append(
                ExtractedQuestion(
                    ...
                    topic_tags=[exam_title, "science", f"grade-{grade}", "nysed-2025"],
                )
            )
    return items
...
def write_outputs(items: list[ExtractedQuestion]) -> None:
    ...
    payload = {
        "source": "NYSED 2025 Grade 5 and Grade 8 Science Released Questions",
    ...
    lines = [
        "# NYSED Science 2025 Raw Collection",
        "",
        "- Source: NYSED 2025 Grade 5 and Grade 8 Science Released Questions",
...
def main() -> None:
    ...
    write_outputs(items)
    print(f"Extracted {len(items)} NYSED 2025 science questions into {OUTPUT_JSON}")
