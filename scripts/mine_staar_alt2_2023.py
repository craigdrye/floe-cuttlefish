...
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "staar_alt2_2023"
OUTPUT_JSON = OUTPUT_DIR / "staar_alt2_2023_questions.json"
OUTPUT_MD = OUTPUT_DIR / "staar_alt2_2023_summary.md"
...
def collect_teacher_guides() -> list[tuple[str, str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if "2023-staar-alt2-" not in lowered_href:
            continue
        if not lowered_href.endswith(".pdf"):
            continue
        if any(token in lowered_href for token in ("student-book", "student-test", "student.pdf")):
            continue
        if not any(token in lowered_href for token in ("teacher", "teacher-book", "teacher-test")):
            continue
...
def infer_exam_title(label: str, url: str) -> str:
    ...
    slug = slug.replace("2023-staar-alt2-", "")
...
def split_question_blocks(text: str) -> list[tuple[int, str]]:
    ...
    cleaned = re.sub(r"Grade \d+ Mathematics - STAAR Alternate 2", " ", cleaned)
    cleaned = re.sub(r"STAAR Alternate 2-?", " ", cleaned)
...
def parse_prompt_and_answer(block: str) -> tuple[str, str]:
    ...
    for raw_line in block.splitlines():
        ...
        if line.startswith("•"):
            line = line[1:].strip()
        if line.lower().startswith("direct the student") or line.lower().startswith("communicate:") or line.lower().startswith("present "):
...
def build_questions() -> list[ExtractedQuestion]:
    ...
    for label, url in collect_teacher_guides():
        ...
        collection_id = f"staar-alt2-2023::{re.sub(r'[^a-z0-9]+', '-', exam_title.lower()).strip('-')}"
        for question_number, block in split_question_blocks(text):
            ...
            items.append(
                ExtractedQuestion(
                    ...
                    topic_tags=[exam_title, subject, grade_band, "staar-alt2-2023"],
                )
            )
    return items
...
def write_outputs(items: list[ExtractedQuestion]) -> None:
    ...
    payload = {
        "source": "Texas STAAR Alternate 2 2023 released teacher guides",
    ...
    lines = [
        "# STAAR Alternate 2 2023 Raw Collection",
        "",
        "- Source: Texas STAAR Alternate 2 2023 released teacher guides",
...
def main() -> None:
    ...
    write_outputs(items)
    print(f"Extracted {len(items)} STAAR Alternate 2 2023 questions into {OUTPUT_JSON}")
