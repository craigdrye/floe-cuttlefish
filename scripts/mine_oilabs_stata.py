...
SCRATCH_REPO = REPO_ROOT / "scratch" / "oilabs_stata_repo"
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "oilabs_stata"
OUTPUT_JSON = OUTPUT_DIR / "oilabs_stata_tasks.json"
OUTPUT_MD = OUTPUT_DIR / "oilabs_stata_summary.md"
REPO_URL = "https://github.com/OpenIntroStat/oilabs-stata.git"
...
def extract_all_tasks() -> list[ExtractedTask]:
    ...
    for rmd_file in sorted(SCRATCH_REPO.glob("*/*.Rmd")):
        ...
        for question_number, block in extract_numbered_blocks(text):
            ...
            if len(prompt) < 25:
                continue
            source_id = f"oilabs-stata::{lab_slug}::{question_number}"
            collection_id = f"oilabs-stata::{lab_slug}"
            tasks.append(
                ExtractedTask(
                    source_id=source_id,
...
def write_outputs(tasks: list[ExtractedTask]) -> None:
    ...
    lines = [
        "# OpenIntro Labs Stata Raw Collection",
        "",
        "- Source: OpenIntro Labs (Stata)",
        "- License: CC BY-SA 3.0",
...
def main() -> None:
    ...
    write_outputs(tasks)
    print(f"Extracted {len(tasks)} oilabs-stata tasks into {OUTPUT_JSON}")
