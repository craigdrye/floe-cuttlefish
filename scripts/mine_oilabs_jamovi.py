...
SCRATCH_REPO = REPO_ROOT / "scratch" / "oilabs_jamovi_repo"
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "oilabs_jamovi"
OUTPUT_JSON = OUTPUT_DIR / "oilabs_jamovi_tasks.json"
OUTPUT_MD = OUTPUT_DIR / "oilabs_jamovi_summary.md"
REPO_URL = "https://github.com/OpenIntroStat/oilabs-jamovi.git"
...
def extract_all_tasks() -> list[ExtractedTask]:
    ...
    for rmd_file in sorted(SCRATCH_REPO.glob("*/*.Rmd")):
        ...
        for question_number, block in extract_numbered_blocks(text):
            ...
            if len(prompt) < 25:
                continue
            source_id = f"oilabs-jamovi::{lab_slug}::{question_number}"
            collection_id = f"oilabs-jamovi::{lab_slug}"
            tasks.append(
                ExtractedTask(
                    source_id=source_id,
...
def write_outputs(tasks: list[ExtractedTask]) -> None:
    ...
    lines = [
        "# OpenIntro Labs Jamovi Raw Collection",
        "",
        "- Source: OpenIntro Labs (jamovi)",
        "- License: CC BY-SA 4.0",
...
def main() -> None:
    ...
    write_outputs(tasks)
    print(f"Extracted {len(tasks)} oilabs-jamovi tasks into {OUTPUT_JSON}")
