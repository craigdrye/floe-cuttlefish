...
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "staar_2021"
OUTPUT_JSON = OUTPUT_DIR / "staar_2021_questions.json"
OUTPUT_MD = OUTPUT_DIR / "staar_2021_summary.md"
...
def collect_test_pairs() -> list[tuple[str, str, str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if "writing-samples" in lowered_href or "revising-and-editing" in lowered_href:
            continue
        if "2021-staar-" not in lowered_href and "staar-2021-" not in lowered_href:
            continue

        title_match = re.match(r"(.+?)\s+(Test Form|Answer Key)\s*$", text)
        if not title_match:
            continue
...
def build_questions() -> list[ExtractedQuestion]:
    ...
    for title, test_url, key_url in collect_test_pairs():
        ...
        collection_id = f"staar-2021::{re.sub(r'[^a-z0-9]+', '-', title.lower()).strip('-')}"
        ...
        for question_number, prompt_text in sorted(questions.items()):
            items.append(
                ExtractedQuestion(
                    ...
                    topic_tags=[title, subject, grade_band, "staar-2021"],
                )
            )
    return items
...
def write_outputs(items: list[ExtractedQuestion]) -> None:
    ...
    payload = {
        "source": "Texas STAAR 2021 released tests",
...
def main() -> None:
    ...
    write_outputs(items)
    print(f"Extracted {len(items)} STAAR 2021 questions into {OUTPUT_JSON}")
