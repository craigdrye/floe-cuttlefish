...
SCRATCH_REPO = REPO_ROOT / "scratch" / "oilabs_tidy_repo"
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "oilabs_tidy"
OUTPUT_JSON = OUTPUT_DIR / "oilabs_tidy_tasks.json"
OUTPUT_MD = OUTPUT_DIR / "oilabs_tidy_summary.md"
REPO_URL = "https://github.com/OpenIntroStat/oilabs-tidy.git"
...
def extract_all_tasks() -> list[ExtractedTask]:
    ...
    for rmd_file in sorted(SCRATCH_REPO.glob("*/*.Rmd")):
        ...
        for question_number, block in extract_numbered_blocks(text):
            ...
            if len(prompt) < 25:
                continue
            source_id = f"oilabs-tidy::{lab_slug}::{question_number}"
            collection_id = f"oilabs-tidy::{lab_slug}"
            tasks.append(
                ExtractedTask(
                    source_id=source_id,
...
def write_outputs(tasks: list[ExtractedTask]) -> None:
    ...
    payload = {
        "source": "OpenIntro Labs (tidyverse)",
    ...
    lines = [
        "# OpenIntro Labs Tidy Raw Collection",
        "",
        "- Source: OpenIntro Labs (tidyverse)",
        "- License: CC BY-SA 4.0",
...
def main() -> None:
    ...
    write_outputs(tasks)
    print(f"Extracted {len(tasks)} oilabs-tidy tasks into {OUTPUT_JSON}")
