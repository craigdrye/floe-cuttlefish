...
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "staar_2025_rationales"
OUTPUT_JSON = OUTPUT_DIR / "staar_2025_rationales.json"
OUTPUT_MD = OUTPUT_DIR / "staar_2025_rationales_summary.md"
...
def filename_to_title(href: str) -> str:
    ...
    slug = slug.replace("2025-staar-", "").replace("-rationale", "").replace("-answer-key", "")
    mapping = {
        "math-3": "Grade 3 Math",
        "rla-3": "Grade 3 RLA",
        "spanish-math-3": "Grade 3 Spanish Math",
        "spanish-rla-3": "Grade 3 Spanish RLA",
        "math-4": "Grade 4 Math",
        "rla-4": "Grade 4 RLA",
        "spanish-math-4": "Grade 4 Spanish Math",
        "spanish-rla-4": "Grade 4 Spanish RLA",
        "math-5": "Grade 5 Math",
        "rla-5": "Grade 5 RLA",
        "science-5": "Grade 5 Science",
        "spanish-math-5": "Grade 5 Spanish Math",
        "spanish-rla-5": "Grade 5 Spanish RLA",
        "spanish-science-5": "Grade 5 Spanish Science",
        "math-6": "Grade 6 Math",
        "rla-6": "Grade 6 RLA",
        "math-7": "Grade 7 Math",
        "rla-7": "Grade 7 RLA",
        ...
        "social-studies-8": "Grade 8 Social Studies",
        "algebra-1": "Algebra I",
        "english-1": "English I",
...
def collect_pairs() -> list[tuple[str, str, str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if "2025-staar-" not in lowered_href or not lowered_href.endswith(".pdf"):
            continue
        title = filename_to_title(href)
        if lowered_href.endswith("-rationale.pdf"):
            rationales[title] = href
        elif lowered_href.endswith("-answer-key.pdf"):
            keys[title] = href
...
def split_rationale_entries(text: str) -> dict[int, str]:
    ...
    cleaned = re.sub(r"STAAR Spring 2025 .*?Rationale[s]?", " ", cleaned)
    cleaned = re.sub(r"\bPage\s+\d+\b", " ", cleaned)
    cleaned = re.sub(r"Item\s+Position\s+Rationale[s]?", " ", cleaned)
    ...
    for index, match in enumerate(matches):
        ...
        if body:
            entries[number] = body[:2600]
    return entries


def split_answer_key_entries(text: str) -> dict[int, str]:
    cleaned = text.replace("\xa0", " ")
    cleaned = re.sub(r"STAAR Spring 2025 .*?Answer Key", " ", cleaned, flags=re.DOTALL)
    cleaned = re.sub(r"Item\s+Position.*?Supporting", " ", cleaned, flags=re.DOTALL)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    matches = list(re.finditer(r"(?<!\d)(\d{1,2})\s+", cleaned))
    entries: dict[int, str] = {}
    for index, match in enumerate(matches):
...
def build_questions() -> list[ExtractedQuestion]:
    ...
    for title, rationale_url, key_url in collect_pairs():
        ...
        collection_id = f"staar-2025-rationale::{re.sub(r'[^a-z0-9]+', '-', title.lower()).strip('-')}"
        ...
        for question_number, prompt_text in sorted(rationale_entries.items()):
            items.append(
                ExtractedQuestion(
                    ...
                    topic_tags=[title, subject, grade_band, "staar-2025-rationale"],
                )
            )
    return items
...
def write_outputs(items: list[ExtractedQuestion]) -> None:
    ...
    payload = {
        "source": "Texas STAAR 2025 rationales",
    ...
    lines = [
        "# STAAR 2025 Rationales Raw Collection",
        "",
        "- Source: Texas STAAR 2025 rationales",
...
def main() -> None:
    ...
    write_outputs(items)
    print(f"Extracted {len(items)} STAAR 2025 rationale items into {OUTPUT_JSON}")
