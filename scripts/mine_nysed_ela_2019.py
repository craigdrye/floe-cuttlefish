...
OUTPUT_DIR = REPO_ROOT / "data" / "raw_collections" / "nysed_ela_2019"
OUTPUT_JSON = OUTPUT_DIR / "nysed_ela_2019_questions.json"
OUTPUT_MD = OUTPUT_DIR / "nysed_ela_2019_summary.md"
USER_AGENT = "floe-rebuild8-content-miner/1.0"

GRADE_URLS = {
    3: "https://www.nysedregents.org/ei/ela/2019/2019-released-items-ela-g3.pdf",
    4: "https://www.nysedregents.org/ei/ela/2019/2019-released-items-ela-g4.pdf",
    5: "https://www.nysedregents.org/ei/ela/2019/2019-released-items-ela-g5.pdf",
    6: "https://www.nysedregents.org/ei/ela/2019/2019-released-items-ela-g6.pdf",
    7: "https://www.nysedregents.org/ei/ela/2019/2019-released-items-ela-g7.pdf",
    8: "https://www.nysedregents.org/ei/ela/2019/2019-released-items-ela-g8.pdf",
}
...
def split_questions(text: str) -> dict[int, str]:
    ...
    cleaned = re.sub(r"New York State administered.*?The purpose of these releases.*?(?=\n\s*\d{1,2}\s+)", " ", cleaned, flags=re.DOTALL)
...
def split_item_map_answers(text: str) -> dict[int, str]:
    ...
    pattern = re.compile(
        r"(?<!\d)(\d{1,2})\s+(Multiple Choice|Constructed Response)\s+([A-D]|n/a)\s+(\d)\s+(NGLS\.ELA\.Content\.NY-[^ ]+)\s+",
        flags=re.IGNORECASE,
    )
...
def build_questions() -> list[ExtractedQuestion]:
    ...
    for grade, url in GRADE_URLS.items():
        ...
        exam_title = f"NYSED 2019 Grade {grade} ELA"
        collection_id = f"nysed-ela-2019::grade-{grade}"
        for question_number, prompt_text in sorted(questions.items()):
            items.append(
                ExtractedQuestion(
                    ...
                    topic_tags=[exam_title, "ela", f"grade-{grade}", "nysed-2019"],
                )
            )
    return items
...
def write_outputs(items: list[ExtractedQuestion]) -> None:
    ...
    payload = {
        "source": "NYSED 2019 Grades 3-8 English Language Arts Released Questions",
...
def main() -> None:
    ...
    write_outputs(items)
    print(f"Extracted {len(items)} NYSED 2019 ELA questions into {OUTPUT_JSON}")
